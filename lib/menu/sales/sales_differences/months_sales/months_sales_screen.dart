...
import 'package:paytap_app/menu/sales/sales_differences/months_sales/months_sales_screen_model.dart';

class MonthsSales extends ConsumerStatefulWidget {
  const MonthsSales({super.key});

  @override
  ConsumerState<MonthsSales> createState() => _MonthsSalesState();
}

class _MonthsSalesState extends ConsumerState<MonthsSales> {
  @override
  void initState() {
    super.initState();
    // 초기화 (한 번만 실행)
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final vm = ref.read(monthsSalesScreenModelProvider.notifier);
      final state = ref.read(monthsSalesScreenModelProvider);
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(monthsSalesScreenModelProvider);
    final vm = ref.read(monthsSalesScreenModelProvider.notifier);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              NavSlider(
                type: 'year',
                startDeName: 'targetYear',
                startDe: state.searchState['targetYear'],
