...
class UserRegisterHistoryItem extends StatelessWidget {
  final Map<String, dynamic> data;
  const UserRegisterHistoryItem({super.key, this.data = const {}});
...
List<Widget> getDealItem(item, {bool isLast = false}) {
  List<Widget> widgets = [
    Row(
    ...
    Row(
      children: [
        Text(
          '${getFlagAndCrdcp(item['flagAndCrdcp'])}${getCardAndIdt(item['cardAndIdt']) != "" ? " ) ${getCardAndIdt(item['cardAndIdt'])}" : ""}',
...
String getFlagAndCrdcp(String flagAndCrdcp) {
  switch (flagAndCrdcp) {
    case '0':
      return '비승인';
    case '1':
...
String getCardAndIdt(String cardAndIdt) {
  switch (cardAndIdt) {
    case '0':
      return '비승인';
    case '1':
