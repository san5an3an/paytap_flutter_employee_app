...
class EasyApprovalHistoryItem extends StatelessWidget {
  final Map<String, dynamic> data;
  const EasyApprovalHistoryItem({super.key, this.data = const {}});
            ...
            child: Column(
              ...
              children: [
                ...child.asMap().entries.map((entry) {
                  ...
                  bool isLast = index == child.length - 1;

                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: getDealItem(item, isLast),
                  );
                }),
              ],
            ),
...
List<Widget> getDealItem(item, bool isLast) {
  return [
    Row(
    ...
    Row(
      children: [
        Text(
          '${item['issCrdcpNm']} ) ${item['payCardNo']}',
