...
class DailyTotalSalesScreenModel extends Notifier<DailyTotalSalesState> {
  final ScrollController scrollController = ScrollController();

  @override
  DailyTotalSalesState build() {
    // dispose 콜백 등록
    ref.onDispose(() {
  ...
  Future<void> initializeData(BuildContext context) async {
    ...
    setLoading(true);
    try {
      await getDailyTotalSales(context);
  ...
  Future<void> refreshData(BuildContext context) async {
    if (!state.isInitialLoading) {
      ...
      try {
        await resetDailyTotalSalesList();
        await getDailyTotalSales(context);
      } catch (e) {
  ...
  //데이터 초기화 하는 함수
  Future<void> resetDailyTotalSalesList() async {
  ...
  //데이터 조회 하는 함수
  Future<void> getDailyTotalSales(BuildContext context) async {
    Map<String, dynamic> data = {...state.searchState};

    Map<String, dynamic> res = await StatisticsService.getAppSaleDaily(data);
...
/// DailyTotalSalesScreenModel Provider
/// Riverpod 3.0.3 - NotifierProvider.autoDispose (권장)
final dailyTotalSalesScreenModelProvider =
    NotifierProvider.autoDispose<
      DailyTotalSalesScreenModel,
      DailyTotalSalesState
    >(DailyTotalSalesScreenModel.new);
